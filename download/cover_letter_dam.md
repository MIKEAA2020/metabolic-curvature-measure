# Cover Letter — Discover Applied Mathematics
...
*Escherichia coli*" for consideration as a Research Article in
Discover Applied Mathematics, via the journal's Snapp submission
system.

The manuscript is written so that the applied line of argument leads:
the question, the measure, the validation, and the empirical
association, with the refinement theory and all proofs collected in
appendices.
...
rerouting geometry. Three results make the work suitable for the
...
2. **A validated, robustly tested gene-sensitivity metric.** From the
   ...
   genes), that it predicts carbon-depletion transcriptional response
   (r = +0.395, p = 2.6 × 10⁻¹⁷; partial r = +0.269). The association
   survives every robustness check in the manuscript: five-decimal
   metric agreement between the two curvature definitions
   (ρ = 0.99998), five lexicographic tie-break conventions with the
   value layer exactly invariant, path variation (the direct per-gene
   rank correlation between the path predictors and the reference-path
   score is ρ = +0.92 and +0.96 over the 424 shared genes),
   cross-platform replication (dissociating on PRECISE, as the layer
   theory predicts), and Glivenko–Cantelli stabilization of the
   underlying event measures across random panels (designed panels
   reproduce them exactly). The association does not propagate to the
   protein layer (proteome r = −0.083 vs. transcript +0.419),
   consistent with cells transcribing the potential for rerouting
   while buffering translation.
...
4. **Named biology.** The induced genes sit in eleven enriched
   regulons led by CRP (Benjamini–Hochberg q = 8.6 × 10⁻¹⁶) at
   whole-operon granularity (nuo, atp, sdh–suc, cyo); the rerouting
   mass concentrates on twenty named branch metabolites of central
   carbon (59.7% of mass, led by F6P, GAP, G6P, DHAP, PEP), giving
   the theory a targeted-metabolomics readout; the buffering null is
   developed into a quantitative deduction that metabolic memory must
   live in fast post-translational state; and a construction-order
   design rule for strain engineering is derived from the same
   geometry (ρ = 0.442, sign agreement 59/66, binomial p = 2.4 ×
   10⁻¹¹). A hand-checkable worked example and a
   metabolic-control-analysis comparison (Kacser–Burns;
   Heinrich–Rapoport) are included for the applied readership.
...
combination of a proved applied-mathematical structure, genome-scale
computation, and an empirical association is, in my view, a direct fit
for Discover Applied Mathematics' readership.

**Disclosure of a related manuscript.** In the interest of full
transparency I disclose that a companion theory manuscript is in
preparation for separate submission: "A Geometric and
Category-Theoretic Theory of Viability: How Sequential Adaptations
Induce Path-Dependent Risk." The two papers are related but distinct
in purpose, and the division of labor is deliberate:
...
- The **submitted manuscript** is the application: the
  ...
  and proofs.
- The **companion manuscript** develops the full categorical
  framework — stratified connections, optic composition, filtered
  colimits, the homotopy-type-theoretic extension — with its own
  proofs and machine verifications. It cites the submitted manuscript
  for the application.
...
`scripts/companion_categorical_v13.tex`, compiled PDF included) and
will be provided as a PDF on request; it is not under consideration
elsewhere.
...
artifacts, verified by a 359-check numeric audit (359/359 PASS) that
...
the condition-dependent *E. coli* proteome). The manuscript carries
the journal's required elements: a 245-word abstract (under the
250-word cap), numeric square-bracket citations, "Fig. n" caption
labels, a CRediT authorship contribution statement, and the competing
interests, funding, generative-AI, and research-data declarations in
the backmatter.
...
- Open access: the journal publishes open access; per the journal's
  current open-access information the article processing charge is
  covered by the publisher's arrangement, so no open-access charges
  are requested.
