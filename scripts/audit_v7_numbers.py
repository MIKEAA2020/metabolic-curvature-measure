...
Outputs: download/deepseek_bridge/v7_number_audit.json (full ledger)
          download/deepseek_bridge/v7_number_audit.md (readable table)
"""
import json
import os
import re
...
check("N-17", "prop:keio-n-source + rem:keio-n-invariance in companion; "
      "abstract carries the three-axis sentence and stays under 265 words",
      "companion_categorical_v3.tex (abstract + Keio section)",
      "labels present: %s / %s; abstract words: %d" % (
      ...
      and 'carbon-source, oxygen, and' in companion)
...
out = {"experiment": "v3 numeric consistency audit (proof-complete revision of "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v7_number_audit.json"), "w") as f:
    json.dump(out, f, indent=1, default=str)

md = ["# v7 numeric consistency audit (nitrogen-source third-axis probe round)", "",
...
with open(os.path.join(DB, "v7_number_audit.md"), "w") as f:
    f.write("\n".join(md))
