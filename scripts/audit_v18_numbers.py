#!/usr/bin/env python3
"""
Numeric consistency audit of journal_manuscript_v10.tex +
...
Outputs: download/deepseek_bridge/v18_number_audit.json (full ledger)
          download/deepseek_bridge/v18_number_audit.md (readable table)
"""
import json
import os
import re
...
check("JP-2", "BMB retarget: author-year citations (natbib round); no "
      ...
      "journal_manuscript_v10_bmb_refs" in tex2 and
...
companion = open(os.path.join(BASE, "scripts", "companion_categorical_v8.tex"),
                 encoding="utf-8").read()
check("KEIO-11", "companion quotes fractions 0.498 and 0.853 (battery)",
      "companion_categorical_v8.tex:prop:poincare-averaging + tab:network-battery",
...
check("O2-15", "prop:keio-o2-limited + rem:keio-o2-invariance in companion; "
      ...
      "companion_categorical_v8.tex (abstract + Keio section)",
...
tex = open(os.path.join(BASE, "scripts",
                        "companion_categorical_v8.tex")).read()
check("R8-ABS-five-axes",
      ...
      "companion_categorical_v8.tex abstract",
...
check("R8-ABS-wordcount",
      ...
      "companion_categorical_v8.tex abstract", f"{nwords} words",
      nwords < 265)
...
out = {"experiment": "v12 numeric consistency audit (symmetric iJO "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v18_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v18_number_audit.md"), "w") as f:
    f.write("\n".join(md))
