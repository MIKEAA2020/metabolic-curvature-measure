...
%  standalone theory companion (scripts/companion_categorical_v6.tex),
...
microarrays, $n=424$), it predicts carbon-depletion transcriptional
response ($r = +0.395$, $p = 2.6 \times 10^{-17}$; partial $r = +0.269$
with reference-level control); the association is invariant under the
metric's lexicographic definition ($\rho = 0.99998$), stable across
five selection rules, and its event measures stabilize at the
Glivenko--Cantelli rate. The association does not propagate to the
...
\begin{remark}[Status of the definition, tie-breaking, and layer assignment]
\label{rem:lock}
...
\label{sec:v6}
All value-layer attribution arms, under the declared protocol of \S\ref{sec:v5} (iJO1366, the reference physiology, $n = 424$): the flux layer $\kmu$ gives $r = +0.395$,
...
measured metric invariance of \S\ref{sec:v5}.
...
\input{journal_manuscript_v7_bmb_refs}
