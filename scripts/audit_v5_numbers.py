#!/usr/bin/env python3
"""
Numeric consistency audit of journal_manuscript_v4.tex + companion_categorical_v3.tex (glucose-only Keio re-run + T7b/T7c round; extends the v4 audit with the Keio section and the new propositions of patch D. Legacy header: 3rd-wave repair round: C2/C5/C1 framing edits, APP-1/2/3/5 corrections, figure directory rename
...
Outputs: download/deepseek_bridge/v5_number_audit.json (full ledger)
          download/deepseek_bridge/v5_number_audit.md (readable table)
"""
import json
import os
import re
...
out = {"experiment": "v3 numeric consistency audit (proof-complete revision of "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v5_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v5_number_audit.md"), "w") as f:
    f.write("\n".join(md))
