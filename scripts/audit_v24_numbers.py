#!/usr/bin/env python3
"""
Numeric consistency audit of journal_manuscript_v16.tex +
...
Outputs: download/deepseek_bridge/v24_number_audit.json (full ledger)
          download/deepseek_bridge/v24_number_audit.md (readable table)
"""
import json
import os
import re
...
tex2 = open(os.path.join(BASE, "scripts",
                         "journal_manuscript_v16.tex")).read()
import math
...
check("JP-2", "BMB retarget: author-year citations (natbib round); no "
      ...
      "journal_manuscript_v16_bmb_refs" in tex2 and
...
bmb_refs = open(os.path.join(BASE, "scripts",
                 "journal_manuscript_v16_bmb_refs.tex")).read()
...
out = {"experiment": "v12 numeric consistency audit (symmetric iJO "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v24_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v24_number_audit.md"), "w") as f:
    f.write("\n".join(md))
