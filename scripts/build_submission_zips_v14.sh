#!/bin/bash
# Build self-contained, compile-ready submission ZIPs for the v14 round:
# main = journal_manuscript_v11.tex (BMB), companion = companion_categorical_v9.tex (TAC).
# Changes vs build_submission_zips_v14.sh (v14 round):
#   - New versioned files only (prior versions untouched):
#     journal_manuscript_v11.tex (from v10) and companion_categorical_v9.tex
#     (from v8), plus their refs files.
#   - Universal Gemini-register round (register-level revision of
#     narrative prose throughout, per the author directive; every
#     numerical claim unchanged, verified by number-integrity multiset
#     diff): main -- intro lead, section leads, result narratives
#     (M1 sweeps, epistasis, regime dial, value carrier, primary
#     association, panel construction, platform 2x2, protein-layer
#     sections), discussion, methods connectives; companion -- abstract
#     connective polish (word count kept under cap), Preliminaries /
#     SAVGS / verdicts / conclusion leads, conclusion four-point
#     structure polished.
#   - Clickable email (mailto:) and ORCID (https://orcid.org/...) in
#     BOTH papers (author footnote / title block).
#   - All numerical claims unchanged; audit_v19_numbers.py 301/301 PASS.
# Each ZIP verified by fresh-dir tectonic compile.
set -euo pipefail
cd /home/z/my-project/metabolic-curvature-measure

STAGE=scripts/overleaf_stage_v14
rm -rf "$STAGE"
mkdir -p "$STAGE/main" "$STAGE/companion"

# ---------- MAIN (Bulletin of Mathematical Biology) ----------
M=$STAGE/main
cp scripts/journal_manuscript_v11.tex          "$M/"
cp scripts/journal_manuscript_v11_bmb_refs.tex "$M/"
cp scripts/journal_manuscript_v11_refs.bib     "$M/"
...
- Overleaf: upload this ZIP via New Project -> Upload Project, set the main
  document to journal_manuscript_v11.tex, recompile. (Run twice so natbib
  citations resolve; Overleaf does this automatically.)
- Command line: pdflatex journal_manuscript_v11.tex  (x2)

## Contents
- journal_manuscript_v11.tex        main source (main document)
- journal_manuscript_v11_bmb_refs.tex  reference list (input by the main file)
- journal_manuscript_v11_refs.bib   underlying BibTeX database (not required
...
rm -rf /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v14 && mkdir -p /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v14/main /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v14/companion
unzip -q download/submission_main_bmb.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v14/main
unzip -q download/submission_companion_tac.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v14/companion
echo "=== fresh-dir compile: main v10 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v14/main && tectonic journal_manuscript_v11.tex 2>&1 | tail -2 && \
 pdfinfo journal_manuscript_v11.pdf | grep Pages)
echo "=== fresh-dir compile: companion v8 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v14/companion && tectonic companion_categorical_v9.tex 2>&1 | tail -2 && \
