...
%  standalone theory companion (scripts/companion_categorical_v14.tex),
...
%  unchanged from v9.
%  Brevity round (v11 -> v12): two register tightenings -- the
%  Positioning paragraph's "We find that it does." shortened to
%  "It does." with the dependent "We also find that" connective
%  dropped, and the "at this point" filler removed from the
%  protein-layer central question. No other change; every
%  numerical claim, citation, and cross-reference unchanged from v11.
%  Abstract round (v12 -> v13): the abstract rebuilt on Gemini's
%  abstract (external_audits/humanized/curvature humanized.txt,
%  L721-725) -- its two-paragraph theory/biology structure, its
%  sentence rhythms and narrative connectives (definition -> "We
%  measure how this map bends" -> evidence -> bridge -> "From this
%  measure" -> "Strikingly" -> "Finally" -> "Our results unify"),
%  in-words glosses for h and L_var, and the plain field-level
%  closer -- replacing the compressed telegraphic form; the
%  protein-layer dissociation now carries its measured numbers
%  (r = -0.083, n = 366), robustness stays one non-defensive
%  sentence per the M1 adjudication, and every number is an
%  audited body claim. No change outside the abstract; body,
%  citations, and cross-references identical to v12.
%  Abstract cap round (v13 -> v14): the abstract trimmed under the
%  author's 255-word cap (269 -> 254 audit-style words) with the
%  register kept on Gemini's abstract: the M1 sweep qualifier dropped
%  (the 93.4--100.0% range already encodes the sweep variability;
%  Gemini's sentence ends there), the statistics conventions "n =" and
%  "evaluated" dropped (Gemini's own plain "424 ... genes"), "fails
%  in general" -> "fails generically" (Gemini's exact word), the
%  boundaries gloss compressed to "the boundaries between active
%  constraint sets", "the measured window" -> "the window", and two
%  function words tightened. Every number, both in-words glosses (h,
%  L_var), and the full connective arc ("We measure how this map
%  bends" -> ... -> "Strikingly" -> "Finally" -> "Our results unify")
%  kept. No change outside the abstract; body, citations, and
%  cross-references identical to v13.
...
%  V17 enrichment round (v16 -> v17), per the V17 revision plan
...
%  versions untouched.
%  V18 JTB-alignment round (v17 -> v18), per the author directive to
...
%  V19 comprehension restructure (v18 -> v19): the paper is re-ordered
...
%  humanized audits (external_audits/humanized/gemini,grok
%  humanized.txt) with Gemini's prose and tone weighted first,
%  Grok's sharpenings second, and v14 as the completeness skeleton.
%  Twelve surgical register edits, all number-preserving except the
%  Discussion standby-model paragraph, which re-quotes three audited
%  body claims (+0.419, -0.083, n = 366): concrete rerouting triggers
%  and the operational-bottleneck gloss in the intro object
%  paragraph; the impulse image in the In-words gloss of Definition
%  2.1; Gemini's opening question for the refinement bridge; the
%  city-street analogy in the exact-counterexample remark; an
%  In-words toll sentence after the regime dichotomy; the value-vs-
%  flux guiding question opening the Alexandrov subsection; the
%  "more than double" decile gloss; the generalization question
%  opening path robustness; the plain tie-break lead and question;
%  the active-set-architecture coda on the tie-break findings; the
%  translational-buffering interpretation paragraph in the
%  Discussion; and the protein-layer-null tightening. The audits'
%  unaudited numbers (decile dispersions, PaxDb p-value, model
%  sizes, the stale 98-check count) and Grok's British spellings
%  were evaluated and rejected. Numbers verified: audit_v23
%  301/301; completeness vs v14 verified by
%  verify_v15_completeness.py.
% =====================================================================
...
\begin{abstract}
\noindent
When nutrients change, cells reroute metabolism through alternative
...
measure of rerouting necessity predicts transcriptional response. It
does. The transcript-level association does not
...
Before turning to computation, we record one structural consequence of the
coupling identity, because it organizes how the empirical results of
Section~\ref{sec:empirical} should be read: the two layers see different
events. Growth sees only the events that change the objective; the flux
layer sees every rerouting, including the zero-cost substitutions that
dominate nutrient transitions. The metric of Definition~\ref{def:kmu} lives
...
vs $0.89$ --- more than double; MWU $1.3 \times 10^{-7}$). The baseline reproduces to the
...
protocol class, not of one rule --- a feature of the network's
...
carries the categorical framework. Neither manuscript depends on the other's claims.
...
$\kmu$. The remaining near-colliding counts are distinct quantities: $435$ genes with at
least one active reaction, $438$ reactions active on the reference physiology, $440$
reaction-level trajectory events, $525$ genes with nonzero metric across conditions,
and $537$ the union of active reactions across conditions --- each deposited with its
...
protocol with TPM log-fold changes. The enrichment statistics of
...
single workstation. An automated suite of $372$ numeric checks
...
exactly; this appendix establishes the \emph{refinement--resolution
...
the two regimes. Throughout this appendix $\Omega \subset \RR^p$ is compact, $p \ge 1$,
...
\href{https://doi.org/10.5281/zenodo.22941018}{DOI 10.5281/zenodo.
