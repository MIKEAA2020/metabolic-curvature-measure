...
companion_categorical_v3.tex (oxygen-limited probe round; extends the
v5 audit -- glucose-only Keio re-run + T7b/T7c -- with the second
perturbation probe: prop:keio-o2-limited + rem:keio-o2-invariance +
the abstract promotion, all traced to the o2_limited artifacts).
...
Outputs: download/deepseek_bridge/v6_number_audit.json (full ledger)
          download/deepseek_bridge/v6_number_audit.md (readable table)
"""
import json
import os
import re
...
check("O2-10", "iML anaerobic: 286->291, +6/-1, kappa 0.985, Jaccard 0.976; WT 0.134",
      ...
      f0['n_essential_new'] == 291 and f0['n_gain_essential'] == 6
      and f0['n_loss_essential'] == 1
      and close(f0['cohen_kappa'], 0.985, 2e-4)
      and close(f0['jaccard'], 0.976, 2e-4)
      and close(m0['wild_type_biomass'], 0.134, 2e-4))
flips_gain = {g['gene_id'] for g in f0['gains']}
flips_loss = {g['gene_id'] for g in f0['losses']}
check("O2-11", "flip identities: gains eno/pgk/gapA/gpmA/gpmM/hemN; loss fabZ",
      ...
      and flips_loss == {'b0180'})
check("O2-12", "anaerobic stats: r +0.260 AUC 0.672; direct +0.118 AUC 0.673; gaps 13",
      "keio_o2_limited_e16_results.json:levels.0.0",
      ...
      close(m0['transitive_calibration']['pearson_r_log_kV_delta_b'], 0.260)
      and close(m0['transitive_calibration']['held_out']['roc_auc'], 0.672, 6e-4)
      and close(m0['direct_arm']['pearson_r'], 0.118)
      and close(m0['direct_arm']['roc_auc'], 0.673, 6e-4)
      and m0['direct_arm']['n_model_gaps_pecE_insilicoN'] == 13)
...
check("O2-13", "flip ratios: glycolysis six 0.70-1.00 aerobically -> 0 anaerobically; fabZ 0 -> 1",
      ...
      and ratios['b0180'] == (0.0, 1.0))
gly = [e for e in f0['subsystem_enrichment_top']
       if e['subsystem'] == 'Glycolysis/Gluconeogenesis']
check("O2-14", "5 of 7 flips in glycolysis/gluconeogenesis, ~30x enrichment",
...
out = {"experiment": "v3 numeric consistency audit (proof-complete revision of "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v6_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v6_number_audit.md"), "w") as f:
    f.write("\n".join(md))
