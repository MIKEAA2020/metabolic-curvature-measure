#!/usr/bin/env python3
"""Symmetric second-engine re-run of the iJO1366 ATPM levels
(40/60/80/100): full canonical (two-stage split-variable pFBA) KO
sweep under scipy.optimize.linprog(method='highs'), mirroring
scripts/atpm_iml_second_engine.py for the iML1515 levels, to test
whether the iJO1366 canonical restoration is engine-invariant.

What the iML round established (keio_atpm_iml_second_engine.json):
labels are fully engine-invariant (kappa 1.000, zero flips), the L1
near-tie itself is engine-invariant, but the kV~200 floor was the
GLPK warm-start path's realization (500/650/782 compensables at kV
in [190,210] collapsed to 1/1,127 under stateless cold-start), and
the transitive r moved from +0.703/+0.655/+0.475 to
+0.9518/+0.9681/+0.9432.

The symmetric iJO questions:
  1. iJO1366 at atpm_40 carries a MASSIVE deposited floor (961/971
     compensables at kV in [190,210], median kV 200.000) that
     vanishes at atpm_60/80/100 (2 genes in band, ~958 at kV <= 0.1)
     -- the mirror image of iML's growing floor.  Does the atpm_40
     floor persist or collapse under the stateless engine?
  2. Are iJO labels engine-invariant (kappa vs the GLPK deposit,
     flip list, max |db|, essential counts 298/298/299/331)?
  3. Is the canonical r itself engine-invariant, in particular the
     atpm_100 value +0.9371 (vs +0.9685/+0.9494/+0.9639 at the
     lower levels) and the plain-vs-canonical restoration story
     (plain r erodes +0.9489 -> +0.4444 while canonical holds)?

Design (engine substitution, not regularization):
  - model state identical to scripts/atpm_stress_keio_probe.py
    (set_ijo_atpm medium: all exchanges closed, glucose -10, O2 -20,
    minerals -1000, EX_tre_e stays closed, ATPM at the level);
  - knockout = the iJO probe's PROPER GPR convention (cobra parsed
    association: every reaction of gene.reactions gets zero bounds;
    zero-GPR genes skipped -- the iJO deposit swept 1,367 rows, i.e.
    every iJO1366 gene has at least one GPR reaction);
  - canonical solve = the repo's independent-engine rule
    (atpm_integrity_fix.highs_pfba, the class validated on iML1515):
    stage 1 max biomass; stage 2 min sum(vp+vm) with v = vp - vm,
    full reaction bounds lb <= v <= ub, biomass >= stage-1 optimum;
    sparse matrices; stateless cold-start solves; deterministic
    presolve-off retry only when the first call fails;
  ...
  - floor census recomputed with one census function applied to BOTH
    ...
    the recorded iJO floor structure before reading the HiGHS answer;
  - L1 block: WT L1 and the compensable dL1 distribution (the iML
    near-tie measurement has no iJO analog in the deposit; the iJO
    GLPK CSVs carry no L1 column, so the block is HiGHS-only), plus
    the top-kV compensable genes (the genuinely forced reroutings,
    the lamB/dhaKLM analog).

Artifacts:
  download/keio_atpm_ijo_second_engine.json
  download/keio_atpm_ijo_second_engine_{key}.csv
Resumable: per-level scratch CSV every 20 newly processed genes.
Smoke mode: --smoke runs WT + 8 genes at one level and exits.
"""
...
# nitrogen_source_keio_probe.py is a flat script: importing it executes
# the whole (resumable, ~7 s) nitrogen round, whose module-level figure
# write clobbers download/keio_nitrogen_source_response.png with the
# legacy figure -- the known sixth-axis defect.  Snapshot BEFORE the
# import, restore in the __main__ finally block.
...
IJO_MINERALS = ["EX_nh4_e", "EX_pi_e", "EX_so4_e", "EX_mg2_e", "EX_ca2_e",
                ...
                "EX_ni2_e", "EX_sel_e"]
# NOTE: the committed atpm_stress_keio_probe.py IJO_MINERALS list carries
# 'EX_zn_e', which does not exist in data/bigg_models/iJO1366.json
# (KeyError under get_by_id).  The run-verified convention is 'EX_zn2_e'
# (scripts/atpm_stuck_gene_diagnostic.py, which reproduced the probe's
# deposited WT biomass under the -1000 mineral quota); the WT assertion
# below re-verifies that agreement at every level.
LEVELS = [40.0, 60.0, 80.0, 100.0]
OUT_JSON = os.path.join(DL, "keio_atpm_ijo_second_engine.json")

DEPOSITED_WT = {40.0: 0.6947359182109596,
                60.0: 0.4962399415792379,
                80.0: 0.2977439649475629,
                100.0: 0.09924798831587155}
...
def set_ijo(model, atpm_lb):
    """Exact copy of the ATPM probe's iJO medium setter."""
...
class HighsCanonical:
    ...
    Byte-for-byte the class validated on iML1515 (the interim round's
    staged dense-vs-sparse debug fixed the biomass-pin row signs)."""
...
def sweep_level(engine, model, atpm, gene_rxn_idx, gene_names,
                smoke=False):
    """Run (or smoke-test) one ATPM level; returns (cdf, b_wt, l1_wt)."""
    key = key_of(atpm)
    set_ijo(model, atpm)
    ...
    scratch = os.path.join(
        DL, f"keio_atpm_ijo_second_engine_{key}_partial.csv")
    rows, done = [], set()
    if os.path.exists(scratch):
        _part = pd.read_csv(scratch)
        rows = _part.to_dict("records")
        done = set(_part["gene_id"].tolist())
        print(f"    resuming: {len(done)} genes already done", flush=True)
    t0 = time.time()
    n_new = 0
    n_target = 8 if smoke else len(model.genes)
    for g in model.genes:
        ...
        if not idxs:
            continue  # the iJO probe's zero-GPR skip (none in practice)
        ...
        if smoke and n_new >= n_target:
            break
    cdf = pd.DataFrame(rows)
    if rows and not smoke:
        pd.DataFrame(rows).to_csv(scratch, index=False)
    return cdf, b_wt, l1_wt
...
def main():
    ...
    smoke_level = 40.0
    out = {"method_note": (
        "Symmetric iJO1366 ATPM-level canonical re-run under the "
        "independent scipy/HiGHS engine (two-stage split-variable "
        "pFBA, cold-start stateless solves, full reaction bounds; the "
        "atpm_integrity_fix convention on sparse matrices), mirroring "
        "the iML1515 second-engine round. Medium, knockout convention "
        "(proper GPR parsing, the iJO deposit's own), kV definition, "
        "label rule, and statistics functions identical to the GLPK "
        "probe (atpm_stress_keio_probe.py); WT reference = the "
        "engine's own fresh canonical solve per level.")}
    if os.path.exists(OUT_JSON) and not smoke:
        try:
    ...
    engine = HighsCanonical(ijo)
    n = engine.n
    rid_to_j = {rid: j for j, rid in enumerate(engine.rids)}
    gene_names = {g.id: (g.name or "") for g in ijo.genes}
    # gene -> KO reaction indices, the iJO probe's PROPER GPR parsing
    t0 = time.time()
    gene_rxn_idx = {}
    for g in ijo.genes:
        gene_rxn_idx[g.id] = [rid_to_j[r.id] for r in g.reactions]
    n_zero_gpr = sum(1 for v in gene_rxn_idx.values() if not v)
    print(f"gene->reaction map built ({time.time()-t0:.0f}s, "
          f"{len(gene_rxn_idx)} genes, {n_zero_gpr} zero-GPR)",
          flush=True)

    if smoke:
        cdf, b_wt, l1_wt = sweep_level(engine, ijo, smoke_level,
                                       gene_rxn_idx, gene_names, smoke=True)
        print("\nSMOKE RESULTS (8 genes, atpm_40):")
        print(cdf[["gene_id", "b_ko", "y_essential", "kV", "solve_src"]]
    ...
    for atpm in LEVELS:
        key = key_of(atpm)
        final_csv = os.path.join(DL, f"keio_atpm_ijo_second_engine_{key}.csv")
        if key in out["levels"] and os.path.exists(final_csv):
            print(f"{key}: already done -- skipping", flush=True)
            continue
        print(f"\n----- iJO1366 ATPM >= {atpm} (HiGHS canonical) -----",
              flush=True)
        cdf, b_wt, l1_wt = sweep_level(engine, ijo, atpm, gene_rxn_idx,
                                       gene_names)
        if len(cdf) < len(ijo.genes):
            print(f"  WARNING: incomplete sweep ({len(cdf)}/"
                  f"{len(ijo.genes)}); scratch retained", flush=True)
        ...
        glpk_csv = os.path.join(DL, f"keio_atpm_pfba_control_{key}.csv")
        ...
        top = comp.nlargest(5, "kV")
        l1_block["top5_compensable_kV_genes"] = [
            {"gene_id": r.gene_id, "gene_name": r.gene_name,
             "kV": float(r.kV), "abs_dL1": abs(float(r.L1) - l1_wt)}
        ...
        out["levels"][key] = {
            ...
            "L1_forced_rerouting": l1_block,
        }
        cdf.to_csv(final_csv, index=False)
        if os.path.exists(scratch_f := os.path.join(
                DL, f"keio_atpm_ijo_second_engine_{key}_partial.csv")):
            os.remove(scratch_f)
    ...
    for atpm in LEVELS:
        ...
        g = lv["vs_glpk"]
        print(f"  iJO ATPM >= {atpm}: r {c['pearson_r_log_kV_delta_b']:+.4f} "
    ...
    print("\nSYMMETRIC SECOND-ENGINE iJO RE-RUN DONE.")
