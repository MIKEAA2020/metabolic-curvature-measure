#!/bin/bash
# Build self-contained, compile-ready submission ZIPs for the v8 round:
# main = journal_manuscript_v5.tex (BMB), companion = companion_categorical_v4.tex (TAC).
# Changes vs build_submission_zips_v7.sh (v7 round):
#   - New manuscript versions (versioned files, prior versions untouched):
#     journal_manuscript_v5.tex (from v4) and companion_categorical_v4.tex
#     (from v3).
#   - Formal-tone revision per the author's directive: declarations in both
#     manuscripts reduced to brief single sentences (Funding: none; no
#     competing interests; AI declaration; ethics: not applicable; author
#     contributions: A.A.); meta-commentary, changelog references to earlier
#     versions, internal-audit and project-diary prose removed from both
#     manuscripts; proofs left at full length.
#   - Figures regenerated without internal experiment codes (V5/V7/V8/E32/
#     AX-8c/E22 labels replaced by manuscript terminology) and with layout
#     fixes: Fig 3 panel (b) bar annotations no longer collide with the
#     panel title; path-robustness panel (b2) same fix; e32 panel (d)
#     annotation no longer clipped. Regenerated from deposited artifacts /
#     deterministic re-runs: all data artifacts byte-identical (e32 differs
#     only in the wall-clock runtime_s field).
#   - Audits after the edits: main audit_v5 98/98 PASS; companion
#     audit_v13 301/301 PASS.
# Each ZIP verified by fresh-dir tectonic compile.
set -euo pipefail
cd /home/z/my-project/metabolic-curvature-measure

STAGE=scripts/overleaf_stage_v8
rm -rf "$STAGE"
mkdir -p "$STAGE/main" "$STAGE/companion"

# ---------- MAIN (Bulletin of Mathematical Biology) ----------
M=$STAGE/main
cp scripts/journal_manuscript_v5.tex          "$M/"
cp scripts/journal_manuscript_v5_bmb_refs.tex "$M/"
...
- Overleaf: upload this ZIP via New Project -> Upload Project, set the main
  document to journal_manuscript_v5.tex, recompile. (Run twice so natbib
  citations resolve; Overleaf does this automatically.)
- Command line: pdflatex journal_manuscript_v5.tex  (x2)

## Contents
- journal_manuscript_v5.tex        main source (main document)
- journal_manuscript_v5_bmb_refs.tex  reference list (input by the main file)
...
- Abstract: 245 words (within the 150-250 guideline).
...
C=$STAGE/companion
cp scripts/companion_categorical_v4.tex    "$C/"
...
## How to compile
- Overleaf: upload this ZIP via New Project -> Upload Project, set the main
  document to companion_categorical_v4.tex, recompile (pdflatex + bibtex +
  pdflatex x2; Overleaf runs this automatically).
- Command line:
    pdflatex companion_categorical_v4.tex
    bibtex   companion_categorical_v4
    pdflatex companion_categorical_v4.tex
    pdflatex companion_categorical_v4.tex

## Contents
- companion_categorical_v4.tex  main source (main document)
...
rm -rf /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v8 && mkdir -p /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v8/main /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v8/companion
unzip -q download/submission_main_bmb.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v8/main
unzip -q download/submission_companion_tac.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v8/companion
echo "=== fresh-dir compile: main v5 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v8/main && tectonic journal_manuscript_v5.tex 2>&1 | tail -2 && \
 pdfinfo journal_manuscript_v5.pdf | grep Pages)
echo "=== fresh-dir compile: companion v4 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v8/companion && tectonic companion_categorical_v4.tex 2>&1 | tail -2 && \
 pdfinfo companion_categorical_v4.pdf | grep Pages)
echo "ALL DONE"
