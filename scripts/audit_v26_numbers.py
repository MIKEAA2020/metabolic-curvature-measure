#!/usr/bin/env python3
"""
Numeric consistency audit of journal_manuscript_v18.tex +
...
companion_categorical_v10.tex (universal Gemini-adoption round:
...
tex2 = open(os.path.join(BASE, "scripts",
                         "journal_manuscript_v18.tex")).read()
import math
...
check("JP-3", "JTB retarget: abstract <= 250 words (live JTB Guide "
...
companion = open(os.path.join(BASE, "scripts", "companion_categorical_v10.tex"),
                 encoding="utf-8").read()
check("KEIO-11", "companion quotes fractions 0.498 and 0.853 (battery)",
      "companion_categorical_v10.tex:prop:poincare-averaging + tab:network-battery",
...
check("O2-15", "prop:keio-o2-limited + rem:keio-o2-invariance in companion; "
      ...
      "companion_categorical_v10.tex (abstract + Keio section)",
...
tex = open(os.path.join(BASE, "scripts",
                        "companion_categorical_v10.tex")).read()
check("R8-ABS-five-axes",
      ...
      "companion_categorical_v10.tex abstract",
...
check("R8-ABS-wordcount",
      ...
      "companion_categorical_v10.tex abstract", f"{nwords} words",
      nwords < 265)
...
check("V17-FM1", "abstract <= 250 audit-style words (JTB cap; "
...
out = {"experiment": "v12 numeric consistency audit (symmetric iJo "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v26_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v26_number_audit.md"), "w") as f:
    f.write("\n".join(md))
