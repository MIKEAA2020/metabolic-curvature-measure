#!/bin/bash
# Build self-contained, compile-ready submission ZIPs for the v16 round:
# main = journal_manuscript_v13.tex (BMB), companion = companion_categorical_v9.tex (TAC).
# Changes vs build_submission_zips_v15.sh (v15 round):
#   - Main only: new versioned file journal_manuscript_v13.tex (from v12)
#     plus its refs copies. Abstract round per the author directive --
#     the abstract rebuilt on Gemini's abstract (two-paragraph
#     theory/biology structure, narrative connectives, in-words glosses
#     for h and L_var, plain field-level closer, protein-layer numbers
#     r = -0.083 / n = 366 now quoted). Body untouched.
#     Companion UNCHANGED (companion_categorical_v9.tex; zero edits).
#   - All numerical claims unchanged (the two abstract additions are
#     audited body claims); audit_v21_numbers.py 301/301 PASS;
#     pattern_sweep_v16 16/16 clean; verify_v13_completeness.py ALL
#     COMPLETE (body byte-identical outside the abstract).
# Each ZIP verified by fresh-dir tectonic compile.
set -euo pipefail
cd /home/z/my-project/metabolic-curvature-measure

STAGE=scripts/overleaf_stage_v16
rm -rf "$STAGE"
mkdir -p "$STAGE/main" "$STAGE/companion"

# ---------- MAIN (Bulletin of Mathematical Biology) ----------
M=$STAGE/main
cp scripts/journal_manuscript_v13.tex          "$M/"
cp scripts/journal_manuscript_v13_bmb_refs.tex "$M/"
cp scripts/journal_manuscript_v13_refs.bib     "$M/"
...
- Overleaf: upload this ZIP via New Project -> Upload Project, set the main
  document to journal_manuscript_v13.tex, recompile. (Run twice so natbib
  citations resolve; Overleaf does this automatically.)
- Command line: pdflatex journal_manuscript_v13.tex  (x2)

## Contents
- journal_manuscript_v13.tex        main source (main document)
- journal_manuscript_v13_bmb_refs.tex  reference list (input by the main file)
- journal_manuscript_v13_refs.bib   underlying BibTeX database (not required
...
- Abstract: approximately 260 words, in two paragraphs (theory; biology).
...
rm -rf /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v16 && mkdir -p /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v16/main /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v16/companion
unzip -q download/submission_main_bmb.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v16/main
unzip -q download/submission_companion_tac.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v16/companion
echo "=== fresh-dir compile: main v13 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v16/main && tectonic journal_manuscript_v13.tex 2>&1 | tail -2 && \
 pdfinfo journal_manuscript_v13.pdf | grep Pages)
echo "=== fresh-dir compile: companion v9 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v16/companion && tectonic companion_categorical_v9.tex 2>&1 | tail -2 && \
