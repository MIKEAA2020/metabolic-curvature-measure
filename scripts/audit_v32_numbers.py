...
Discover Applied Mathematics round: the causal-coherence/prose-alignment review findings F2-F6 applied as a light touch-up -- F3 the direct trajectory rank
...
sections with the two-sevens disambiguation, and one
experiment-framing sentence in each computational section; every
theorem, proof, number, and section order unchanged; abstract 264
words < 265 cap with the six-axis sentence intact; main v19
unchanged; extends the v28 audit of)companion_categorical_v15.tex (V11 companion-alignment round: the
categorical subsection removed from the v19 main body is delegated to
the companion -- every load-bearing element already lives there, so
the removal is clean delegation; the companion's two cross-paper
sentences updated to the v19 division of labor (the application paper
now cites this paper and records the division of labor in one
Discussion paragraph; it no longer states even brief adapted
categorical definitions), plus the header Relation comment and the
byte-identical companion_refs_v11.bib pointer; no numerical claim,
theorem, proof, or section touched; main v19 unchanged; extends the
v27 audit of)(v19 comprehension restructure, BMB target, after the JTB desk
...
journal_manuscript_v22_dam_refs.tex; every v18 number unchanged,
...
(v17 enrichment round: implements the V17 revision plan
(commits 4776f2d + 74934d0) --- Sec. 2 worked example, Sec. 4
wall coordinates + interior architecture + construction order,
Sec. 5 anatomy of one switch, Discussion MCA + memory substrate,
bio-anchored abstract at the 255 cap; all new numbers from
download/v17_insight_substantiation.json and
download/v17_worked_example_verification.json, audited by the
V17 check block; every v16 number unchanged. Extends the v24
audit of journal_manuscript_v16.tex +
companion_categorical_v15.tex (universal Gemini-adoption round:
...
the
scipy/HiGHS stateless re-run of the iML1515 ATPM levels
(keio_atpm_iml_second_engine -- labels engine-invariant at kappa
1.000, the near-tie itself engine-invariant, the kV floor measured
as the deposited simplex path's realization, the transitive
association restored to +0.952/+0.968/+0.943), the PGI
supply-range correction (45-201, the unrecorded iML1515 nh4_-2.5
cell certified at 158.316), the post-patch compensable-count
correction (1,127), the nitrogen-figure clobber repair, and the
patch-I amendments; all traced to committed artifacts).
...
Outputs: download/deepseek_bridge/v24_number_audit.json (full ledger)
          download/deepseek_bridge/v25_number_audit.md (readable table)
"""
import json
import os
import re
...
tex2 = open(os.path.join(BASE, "scripts",
                         "journal_manuscript_v22.tex")).read()
import math
...
check("JP-2", "DAM retarget: numeric square-bracket citations "
      ...
      "journal_manuscript_v22_dam_refs" in tex2 and
...
check("JP-5", "BMB-style references: 29 entries, alphabetical with "
      ...
      n_bib == 29 and has_labels and cited_keys <= set(ref_keys) and
...
companion = open(os.path.join(BASE, "scripts", "companion_categorical_v15.tex"),
                 encoding="utf-8").read()
check("KEIO-11", "companion quotes fractions 0.498 and 0.853 (battery)",
      "companion_categorical_v15.tex:prop:poincare-averaging + tab:network-battery",
...
check("O2-15", "prop:keio-o2-limited + rem:keio-o2-invariance in companion; "
      ...
      "companion_categorical_v15.tex (abstract + Keio section)",
...
check("N-17", "prop:keio-n-source + rem:keio-n-invariance in companion; "
      "abstract carries the six-axis sentence (round 10: + the "
      "non-medium maintenance axis) and stays under 265 words",
      "companion_categorical_v15.tex (abstract + Keio section)",
      "labels present: %s / %s; abstract words: %d" % (
      ...
      and 'carbon, oxygen, nitrogen,' in companion
      and 'non-medium maintenance stress' in companion)
...
print(f"[{'PASS' if checks[-1]['status'] == 'PASS' else 'FAIL'}] "
      f"N-17: six-axis abstract present, word count {_nwordsN} < 265")
...
tex = open(os.path.join(BASE, "scripts",
                        "companion_categorical_v15.tex")).read()
check("R8-ABS-five-axes",
      "abstract: 'robust across six axes: carbon, oxygen, nitrogen, "
      "phosphate, and iron supply plus non-medium maintenance stress'",
      "companion_categorical_v15.tex abstract",
      "six axes" in tex and "maintenance stress" in tex,
      "robust across six axes" in tex
      and "maintenance stress" in tex[tex.find("Abstract."):
                                        tex.find("\\textbf{Keywords:}")])
i0 = tex.find("\\textbf{Abstract.}")
j0 = tex.find("\\textbf{Keywords:}", i0)
...
check("R8-ABS-wordcount",
      ...
      "companion_categorical_v15.tex abstract", f"{nwords} words",
      nwords < 250)
...
check("R9-TEX-table-rows",
      ...
      "restored range [+0.80,+0.96]; 'Five features matter'",
      "companion_categorical_v15.tex",
      ...
      and "Five features matter" in tex)
...
check("S-5", "atpm_100 gains +42 on iJO: atp operon (9), cyo (4), nuo "
      ...
      and all(b in _gids for b in
              ["b0429", "b0430", "b0431", "b0432", "b2276", "b2282",
               ...
               "b3875"])
      and any(e["n_flip"] == 26 and close(e["expected"], 3.0, 0.05)
...
check("S-8", "canonical iML +0.703/+0.655/+0.475 (AUC 0.992/0.992/"
      ...
      and close(_cr(actrl['iml_levels'], "atpm_100"), 0.475)
...
check("S-9", "integrity: 6 corrupted calls adjudicated (plain b0180/"
      ...
      and all(abs(v["b_highs_true"]) < 1e-9
              for v in adjr["verdicts"]
              if f"{v['model']}:{v['gene_id']}" in {
                  f"{c['model']}:{c['gene_id']}"
                  for c in adjr["patch"]["corrections"]})
...
# ------------- v11 round: proof-read + second-engine ------------------
v11_eng = json.load(open(os.path.join(
    DL, "keio_atpm_iml_second_engine.json")))
v11_gL = json.load(open(os.path.join(
    DL, "keio_atpm_pfba_control.json")))["iml_levels"]
v11_lv = ["atpm_60", "atpm_80", "atpm_100"]

check("P-1", "second-engine WT biomass matches the deposited level WT "
      "(three levels, |diff| < 1e-6)",
      "keio_atpm_iml_second_engine.json",
      "; ".join(f"{k}: {v11_eng['levels'][k]['wild_type_biomass']:.6f} vs "
                f"{v11_gL[k]['wild_type_biomass']:.6f}" for k in v11_lv),
      all(abs(v11_eng["levels"][k]["wild_type_biomass"]
              - v11_gL[k]["wild_type_biomass"]) < 1e-6 for k in v11_lv))

check("P-2", "labels engine-invariant: kappa 1.000, zero flips, "
      "essential counts 295/297/329 identical to GLPK",
      "keio_atpm_iml_second_engine.json:vs_glpk",
      "; ".join(f"{k}: kappa "
                f"{v11_eng['levels'][k]['vs_glpk']['label_kappa']:.4f}, "
                f"flips {len(v11_eng['levels'][k]['vs_glpk']['label_flips'])}, "
                f"{v11_eng['levels'][k]['n_essential']}/"
                f"{v11_eng['levels'][k]['n_essential_glpk']}"
                for k in v11_lv),
      all(v11_eng["levels"][k]["vs_glpk"]["label_kappa"] == 1.0
          and len(v11_eng["levels"][k]["vs_glpk"]["label_flips"]) == 0
          and v11_eng["levels"][k]["n_essential"]
          == v11_eng["levels"][k]["n_essential_glpk"] for k in v11_lv))

check("P-3", "maximum biomass discrepancy across engines <= 1e-7 at "
      "all three levels",
      "keio_atpm_iml_second_engine.json:vs_glpk.max_abs_db_ko",
      "; ".join(f"{k}: "
                f"{v11_eng['levels'][k]['vs_glpk']['max_abs_db_ko']:.2e}"
                for k in v11_lv),
      all(v11_eng["levels"][k]["vs_glpk"]["max_abs_db_ko"] <= 1e-7
          for k in v11_lv))

check("P-4", "manuscript second-engine r values +0.952/+0.968/+0.943 "
      "(prop, fifth feature, remark, table caption)",
      "keio_atpm_iml_second_engine.json",
      "; ".join(f"{k}: "
                f"{v11_eng['levels'][k]['transitive_calibration']['pearson_r_log_kV_delta_b']:+.4f}"
                for k in v11_lv),
      all(close(v11_eng["levels"][k]["transitive_calibration"]
                ["pearson_r_log_kV_delta_b"], a, 5e-4)
          for k, a in zip(v11_lv, [0.952, 0.968, 0.943]))
      and "+0.952" in texS and "+0.968" in texS and "+0.943" in texS)

check("P-5", "manuscript second-engine AUC 0.992/0.992/0.984",
      "keio_atpm_iml_second_engine.json",
      "; ".join(f"{k}: "
                f"{v11_eng['levels'][k]['transitive_calibration']['held_out']['roc_auc']:.4f}"
                for k in v11_lv),
      all(close(v11_eng["levels"][k]["transitive_calibration"]["held_out"]
                ["roc_auc"], a, 5e-4)
          for k, a in zip(v11_lv, [0.992, 0.992, 0.984])))

check("P-6", "stateless engine floor census: 1 of 1127 compensables in "
      "the 190-210 floor band at every level",
      "keio_atpm_iml_second_engine.json:floor_census_highs",
      "; ".join(f"{k}: "
                f"{v11_eng['levels'][k]['floor_census_highs']['n_floor_190_210']}"
                f"/{v11_eng['levels'][k]['floor_census_highs']['n_compensable']}"
                for k in v11_lv),
      all(v11_eng["levels"][k]["floor_census_highs"]["n_floor_190_210"] == 1
          and v11_eng["levels"][k]["floor_census_highs"]["n_compensable"]
          == 1127 for k in v11_lv))

v11_gfc = {}
for k in v11_lv:
    v11_df = pd.read_csv(os.path.join(
        DL, f"keio_atpm_pfba_control_iml_{k}.csv"))
    v11_bwt = float(v11_df.b_wt.iloc[0])
    v11_c = v11_df[v11_df.b_ko >= 0.999 * v11_bwt]
    v11_gfc[k] = (int(len(v11_c)),
                  int(((v11_c.kV >= 190) & (v11_c.kV <= 210)).sum()))
check("P-7", "GLPK floor census recomputed from the committed CSVs: "
      "500/650/782 of 1127; manuscript reads '782 of 1,127' "
      "(the 1,129 was the pre-integrity-patch measurement)",
      "keio_atpm_pfba_control_iml_atpm_*.csv",
      "; ".join(f"{k}: {v11_gfc[k][1]}/{v11_gfc[k][0]}" for k in v11_lv),
      all(v11_gfc[k][0] == 1127 for k in v11_lv)
      and v11_gfc["atpm_60"][1] == 500 and v11_gfc["atpm_80"][1] == 650
      and v11_gfc["atpm_100"][1] == 782
      and "$782$ of\n$1{,}127$" in texS and "1{,}129" not in texS)

v11_lb = {}
v11_big = {}
for k in v11_lv:
    v11_df = pd.read_csv(os.path.join(
        DL, f"keio_atpm_iml_second_engine_{k}.csv"))
    v11_c = v11_df[v11_df.b_ko >= 0.999 * float(v11_df.b_wt.iloc[0])]
    v11_big[k] = sorted(v11_c[v11_c.kV > 190].gene_id.tolist())
    v11_lb[k] = {r.gene_id: round(float(r.kV), 2)
                 for r in v11_c.itertuples()
                 if r.gene_id in ("b4036", "b1200", "b1199", "b1198",
                                  "b3946", "b0825")}
check("P-8", "the stateless engine's only floor-band compensable is lamB "
      "(b4036, kV 200.0 at every level); the five-gene rerouting block "
      "dhaKLM + fsaA/fsaB at kV 162-416 (manuscript wording)",
      "keio_atpm_iml_second_engine_atpm_*.csv",
      json.dumps(v11_lb),
      all(v11_lb[k].get("b4036") is not None
          and abs(v11_lb[k]["b4036"] - 200.0) < 0.05 for k in v11_lv)
      and v11_big["atpm_60"] == ["b4036"]
      and v11_big["atpm_80"] == ["b0825", "b1198", "b1199", "b1200",
                                 "b3946", "b4036"]
      and v11_big["atpm_100"] == ["b0825", "b1198", "b1199", "b1200",
                                  "b3946", "b4036"]
      and abs(v11_lb["atpm_100"]["b1200"] - 416.38) < 0.5
      and abs(v11_lb["atpm_60"]["b1200"] - 162.0) < 0.5
      and "\\emph{lamB}, $kV=200.0$ at every level" in texS)

v11_nt = v11_eng["levels"]["atpm_100"]["L1_neartie"]
check("P-9", "near-tie engine-invariant: HiGHS WT L1 and b0870 dL1 "
      "reproduce the GLPK measurement (747.682605 / -8.57e-4) to 2e-5",
      "keio_atpm_iml_second_engine.json:L1_neartie",
      f"WT L1 {v11_nt['wt_L1']:.7f}; dL1 {v11_nt['dL1_absolute']:.3e}",
      abs(v11_nt["wt_L1"] - 747.6826047) < 2e-5
      and abs(v11_nt["dL1_absolute"] - (-8.5723e-4)) < 2e-5)

check("P-10", "engine bracket [+0.475,+0.943] and path-dependence "
      "wording present; 'caps near' removed",
      "companion_categorical_v15.tex",
      "bracket present: %s" % ("$[+0.475,+0.943]$" in texS),
      "$[+0.475,+0.943]$" in texS
      and "deposited simplex path" in texS and "caps near" not in texS)

v11_ph = json.load(open(os.path.join(
    DL, "keio_phosphate_limited_e12_results.json")))
v11_pgi_max = max(v11_ph["levels"][kk]["degeneracy"]["fva_widths"]["PGI"]
                  for kk in v11_ph["levels"])
v11_pgi_iml = json.load(open(os.path.join(
    DL, "keio_iml_nh4_pgi_width.json")))["PGI_width"]
check("P-11", "supply-axis PGI range 45-201 (max deposited 201.492; the "
      "unrecorded iML nh4_-2.5 cell certified 158.316; '45-202' and the "
      "bare 'every supply level' qualifier removed)",
      "keio_phosphate_limited_e12_results.json + "
      "keio_iml_nh4_pgi_width.json",
      f"max {v11_pgi_max:.3f}; iML nh4_-2.5 {v11_pgi_iml}",
      close(v11_pgi_max, 201.492, 1e-3)
      and close(v11_pgi_iml, 158.316, 1e-3)
      and "45$--$201" in texS and "45$--$202" not in texS
      and "at every nitrogen-, phosphate-, and iron-limited level" in texS)

check("P-12", "infeasible KO counts identical across engines (14/16/46)",
      "keio_atpm_iml_second_engine.json:solve_sources",
      "; ".join(f"{k}: {v11_eng['levels'][k]['solve_sources']}"
                for k in v11_lv),
      all(v11_eng["levels"][k]["solve_sources"].get("infeasible") == n
          for k, n in zip(v11_lv, [14, 16, 46])))

v11_sz = os.path.getsize(os.path.join(
    DL, "keio_nitrogen_source_response.png"))
check("P-13", "nitrogen 4-panel publication figure present (157,682 "
      "bytes; the sixth-axis commit had shipped the probe-import "
      "clobber at 129,624 -- regenerated byte-identical)",
      "download/keio_nitrogen_source_response.png",
      f"{v11_sz} bytes",
      v11_sz == 157682)

check("P-14", "patch I present: five amendments, no regressions "
      "('45--202', '1,129', 'caps near', 'only partially sufficient' "
      "all gone)",
      "companion_categorical_v15.tex",
      "edits present: %s" % all(t in texS for t in
          ["against $45$--$201$", "stateless\nsecond engine resolves",
           "$782$ of\n$1{,}127$", "$[+0.475,+0.943]$", "\\emph{lamB}"]),
      all(t in texS for t in
          ["against $45$--$201$", "stateless\nsecond engine resolves",
           "$782$ of\n$1{,}127$", "$[+0.475,+0.943]$", "\\emph{lamB}"])
      and all(t not in texS for t in
              ["45$--$202", "1{,}129", "caps near",
...
check("V17-FM1", "abstract <= 249 audit-style words (DAM cap; "
      ...
      "journal_manuscript_v22.tex abstract", f"{nwords17} words",
...
check("V17-FM2", "refs: 29 entries with kacser1973 + heinrich1974 "
      ...
      "journal_manuscript_v22_dam_refs.tex", f"{n17} entries",
...
check("V18-FM2", "keywords: 6 terms (BMB 4-6); 'path dependence' "
      ...
      "journal_manuscript_v22.tex keywords", f"{kw_terms18} terms",
      kw_terms18 == 6 and
      "path dependence" not in kw_line18.group(1))

# --- V19 structural-restructure gates (comprehension round) ---
...
check("V19-FM3", "structural restructure: categorical subsection out, "
      ...
      "journal_manuscript_v22.tex structure", "5 sub-gates",
...
check("V19-FM4", "plain title retained; v20 narrows it to the measured "
      ...
      "journal_manuscript_v22.tex front matter", "title gates",
...
check("V19-FM5", "intro claim structure rebuilt: five-finding "
      ...
      "journal_manuscript_v22.tex intro (vs the v18 claim list)",
      "intro gates",
...
check("V20-1", "F3: direct trajectory rank correlation reported "
      ...
      "v20_path_rank_stability.json + journal_manuscript_v22.tex",
      "artifact %.4f / %.4f (n %d)" % (
...
check("V20-2", "F4: intro Glivenko--Cantelli sentence carries the "
      ...
      "journal_manuscript_v22.tex intro",
...
check("V20-4", "F6: translation-buffering mechanism hedged at both "
      ...
      "journal_manuscript_v22.tex abstract + intro",
...
check("V20-5", "DAM formatting: numeric square-bracket citations in "
      ...
      "journal_manuscript_v22.tex + companion_categorical_v15.tex",
...
check("V20-7", "companion keywords: 6 terms (Springer 4-6 range); "
      ...
      "companion_categorical_v15.tex keywords", f"{_kw13_terms} terms",
...
_expected_total = len(checks) + 2 + 7 + 6  # V20-9 (self) + V20-10 (refs) + V21-1..V21-7 + V22-1..V22-6
check("V20-9", "Reproducibility text: the stated audit-suite size "
      "equals the v32 ledger size (no stale check count)",
      "journal_manuscript_v22.tex Reproducibility + this audit",
...
check("V20-10", "v20 refs list: 29 entries preserved (kacser1973 + "
      ...
      "changed)",
      "journal_manuscript_v22_dam_refs.tex", "entry census",
...
check("V21-1", "final-prose M1/M2: memory-subsection transcript token "
      ...
      "journal_manuscript_v22.tex Discussion (memory subsection)",
...
check("V21-3", "final-prose C1: the where-clause of the piecewise-"
      ...
      "companion_categorical_v15.tex thm:stratified-holonomy",
...
check("V21-4", "final-prose C2: the two-contractions remark's state "
      ...
      "excludes)",
      "companion_categorical_v15.tex rem:two-contractions",
      "state space",
...
check("V21-5", "final-prose C4: companion figure captions carry the "
      ...
      "references remain",
      "companion_categorical_v15.tex preamble + text",
...
check("V21-6", "final-prose C3: the Ito-expansion proof's variance "
      ...
      "companion_categorical_v15.tex lem:ito-expand proof",
      "punctuation",
...
check("V21-7", "v22 refs + v15 bib carried unchanged: the 29-entry "
      "list; companion_refs_v15.bib byte-identical to "
...
out = {"experiment": "V22/V15 numeric consistency audit (Zenodo-archive "
       ...
       "manuscript_defects_found": [
           ...
           "D-P1 (v11 round): prop:keio-atpm PGI supply range 45-202 -> "
           "45-201 (max deposited 201.492; qualifier tightened to the "
           "nitrogen/phosphate/iron levels)",
           "D-P2 (v11 round): fifth-feature compensable count 1,129 -> "
           "1,127 (post-integrity-patch census; 1,129 was the pre-patch "
           "measurement)",
           "D-P3 (v11 round): 'r caps near +0.48' -> measured engine "
           "bracket [+0.475, +0.943] (stateless second engine); 'only "
           "partially sufficient' -> 'the canonical vertex itself is "
           "only weakly determined'",
           "D-P4 (v11 round): nitrogen 4-panel figure shipped clobbered "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v32_number_audit.json"), "w") as f:
    json.dump(out, f, indent=1, default=str)

md = ["# V22/V15 numeric consistency audit (Zenodo-archive round: archival DOIs wired into both availability statements; v31 ledger retargeted to journal_manuscript_v22.tex + companion_categorical_v15.tex)", "",
      ...
      "'as large as 8.2%'); v11 round: D-P1 (PGI range 45-201), D-P2 "
...
with open(os.path.join(DB, "v32_number_audit.md"), "w") as f:
    f.write("\n".join(md))
