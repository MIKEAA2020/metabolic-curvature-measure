#!/usr/bin/env python3
"""
Numeric consistency audit of journal_manuscript_v6.tex +
companion_categorical_v5.tex (symmetric iJO1366 second-engine +
...
tex2 = open(os.path.join(BASE, "scripts",
                         "journal_manuscript_v6.tex")).read()
import math
...
check("JP-2", "BMB retarget: author-year citations (natbib round); no "
      ...
      "journal_manuscript_v6_bmb_refs" in tex2 and
...
bmb_refs = open(os.path.join(BASE, "scripts",
                 "journal_manuscript_v6_bmb_refs.tex")).read()
...
companion = open(os.path.join(BASE, "scripts", "companion_categorical_v5.tex"),
                 encoding="utf-8").read()
check("KEIO-11", "companion quotes fractions 0.498 and 0.853 (battery)",
      "companion_categorical_v5.tex:prop:poincare-averaging + tab:network-battery",
...
check("O2-15", "prop:keio-o2-limited + rem:keio-o2-invariance in companion; "
      ...
      "companion_categorical_v5.tex (abstract + Keio section)",
...
tex = open(os.path.join(BASE, "scripts",
                        "companion_categorical_v5.tex")).read()
check("R8-ABS-five-axes",
      ...
      "companion_categorical_v5.tex abstract",
...
check("R8-ABS-wordcount",
      ...
      "companion_categorical_v5.tex abstract", f"{nwords} words",
      nwords < 265)
...
out = {"experiment": "v12 numeric consistency audit (symmetric iJO "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v14_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v14_number_audit.md"), "w") as f:
    f.write("\n".join(md))
