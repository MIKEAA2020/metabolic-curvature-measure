#!/usr/bin/env python3
"""
Numeric consistency audit of journal_manuscript_v8.tex +
...
check("JP-2", "BMB retarget: author-year citations (natbib round); no "
      ...
      "journal_manuscript_v8_bmb_refs" in tex2 and
...
check("R8-ABS-wordcount",
      ...
      "companion_categorical_v7.tex abstract", f"{nwords} words",
      nwords < 265)
...
out = {"experiment": "v12 numeric consistency audit (symmetric iJO "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v16_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v16_number_audit.md"), "w") as f:
    f.write("\n".join(md))
