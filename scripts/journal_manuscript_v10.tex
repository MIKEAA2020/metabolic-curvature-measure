% =====================================================================
%  journal_manuscript_v10.tex -- main manuscript.
%  Target journal: Bulletin of Mathematical Biology (Springer).
%  Author-year citations (natbib); reference list in
%  journal_manuscript_v10_bmb_refs.tex. Long technical proofs are
...
%  standalone theory companion (scripts/companion_categorical_v8.tex),
...
A genome-scale metabolic network must keep its fluxes coordinated
while nutrient supplies, oxygen tension, and enzyme capacities
change around it. Flux balance analysis and its extensions predict
the phenotypic end-states of this coordination by constrained
...
environment shifts --- has remained largely unaddressed, and for a
structural reason: piecewise-affine maps have vanishing second
derivatives almost everywhere, so smooth calculus cannot describe
rerouting; the response concentrates on discrete transitions across
constraint boundaries, where the optimal pathway pattern switches.
The natural language for this question is geometric measure theory.
...
carrier of the network's rerouting geometry.
...
Fix the parametric form once. A network of $m$ reactions with
stoichiometric matrix $S \in \RR^{k \times m}$ operates under steady
state $Sv = 0$ and bounds $\ell(\theta) \le v \le u(\theta)$ whose
data depend affinely on a parameter vector $\theta$ (uptake limits,
knockdown scalings); the optimum solves
...
partitions parameter space into finitely many polyhedral chambers on
which the active set is constant and the optimal flux is affine in
$\theta$; the codimension-one facets between adjacent chambers are
the switching boundaries. When the parameter path crosses one, the
optimal flux vector changes slope abruptly. The following definition
collects
all of these slope changes into a single object. Read $u$ as the
...
The measure $\mu$ lives on parameter space; the biology enters when
its mass is integrated along the trajectories that environments
actually take, and the accumulated mass is attributed to genes.
...
\label{rem:lock}
The definition of $\kmu$ is fixed as stated: all empirical claims of
this manuscript use it in exactly this form. (i) The carrier is the flux-layer signed crease measure
...
Parametric linear programming also produces a scalar object: the
optimal value $\Phi(\theta)$, the maximal growth rate in the metabolic
setting. The value function is canonical --- it does not depend on
which optimal flux the solver returns --- and classical convex
analysis attaches to it a curvature measure of its own, which this
subsection develops.
...
The two layers are not independent. The next results record the exact
relation: the value measure is the objective's contraction of the
flux measure, and the flux-layer mass that the value layer cannot see
is precisely the degenerate rerouting inside optimal faces.
...
The definitions and results of \S\ref{sec:measure} are
measure-theoretic, and the empirical claims rest on them alone. The
categorical reading summarized in this subsection motivates the
active-set curvature interpretation and is developed in full ---
...
Section~\ref{sec:measure} treated the discrete measure exactly. This
section asks the refinement question: as a triangulation of parameter
space is refined, does the discrete measure converge to the smooth
curvature of a smooth response map? The answer splits --- weakly and
cell-wise yes, in total variation no --- and a resolution parameter
$\sigma$ separates the two regimes. Throughout this section $\Omega \subset \RR^p$ is compact, $p \ge 1$,
...
by sequences of LP value functions, and is stated as a conjecture.
...
and~\ref{sec:theoremB} are operational: they refer to the active set
of a solved linear program, and can therefore be tested directly on
genome-scale models. Each subsection below reports one such test.


\subsection{Active-set curvature verification}
\label{sec:m1}
We first verify the support claim: second-order response mass should
concentrate on active-set switches. Thirteen parameter sweeps (glucose and oxygen uptake; capacity
...
tie-break weights resolving the vertex degeneracy of plain pFBA), with
...
perturbations should interact through it.
...
the smoothing scale $\sigma$ and dialing one against the other.
...
the environment actually tightens? The answer comes in layers --- a
robust transcriptional association, an explicit negative result at
the protein layer, and robustness checks across trajectories,
tie-break conventions, and panels.
...
deposited values exactly (per-gene maxima, the biomass-deficit
maximum, and the unmasked/masked panel correlations to four
decimals); (iii) distinctness --- the panel shares exactly one b-number with the $\kk$ panel ($b2097$); (iv) non-zero variation ---
every panel gene has $\kappa_V$ variation $> 0$ over T1--T8
($435/435$). The geometric metric $\kk$ is weak at the panel
level ($r$ from $-0.063$ unmasked to $+0.084$ masked); the
measure-theoretic $\kmu$ (\S\ref{sec:v5}) is the recalibrated
...
$\rho_S = +0.414$ (full panel, zeros included). Because highly
expressed metabolic enzymes often show broader regulatory dynamic
ranges, the raw association could partly reflect baseline expression
rather than rerouting; residualizing the M3D reference level leaves
the association intact (partial $r = +0.269$,
$p = 1.8 \times 10^{-8}$), and the extreme deciles separate cleanly
(top versus bottom response $1.92$ vs $0.89$; MWU
$1.3 \times 10^{-7}$). Baseline reproduced to the
...
replication with the declared metric: the carbon-switch arms replicate
on the same platform (M3D microarray: glycerol $+0.195$, acetate
$+0.166$, proline $+0.175$, switch-MAX $+0.206$; $n = 424$ genes, all
...
association is specific to the carbon-exhaustion class and the
platform-matched design; the cross-platform dissociation is reported
as a negative result.

\subsection{The layer decision}
\label{sec:v6}
All value-layer attribution arms, under the declared protocol of \S\ref{sec:v5} (iJO1366, the reference physiology, $n = 424$ genes): the flux layer $\kmu$ gives $r = +0.395$,
...
classes), executed with the geometric $\kk$ metric (rank-equivalent
...
at the protein layer answer it (PaxDb integrated
...
consistent with buffering: rerouting capacity is carried at the
transcript level, and the translational cost is deferred until a
rerouting bottleneck actually demands it (\S\ref{sec:e27}).
...
atomic object. Outside the window --- $\sigma$ below the mesh scale
or above the variation scale --- the smooth representation breaks
down, and the system is properly described by its discrete
active-set atoms. Open items: Conjecture~\ref{conj:bridge} (general complex stability) and second differences on measured time courses (under the $(\varepsilon, \sigma)$ design law). The statistical form of the stabilization question is settled here (\S\ref{sec:e32}): random panels are Glivenko--Cantelli objects and designed panels reproduce the event measure exactly; the deterministic refinement-sequence form (refinement along explicit model-family sequences) remains open.
...
measured metric invariance of \S\ref{sec:v5}. Because cells
routinely exploit alternative pathways of equal growth during
nutrient transitions, most metabolic rerouting is objective-invisible
(the zero-cost substitutions of Theorem~\ref{thm:coupling}(i)). The
empirical
association reflects the separation: gene expression changes correlate
with the flux-layer metric $\kmu$ ($r = +0.395$) but show no association
with value-layer derivatives (shadow-price arm $r = +0.032$, $p = 0.93$,
$n = 51$); transcriptional response tracks rerouting, and rerouting is a
flux-layer quantity.
...
\input{journal_manuscript_v10_bmb_refs}
