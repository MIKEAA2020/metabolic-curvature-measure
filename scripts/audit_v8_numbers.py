...
check("N-17", "prop:keio-n-source + rem:keio-n-invariance in companion; "
      "abstract carries the four-axis sentence (round 8: + phosphate) and "
      ...
      and 'and phosphate perturbations' in companion)
...
out = {"experiment": "v3 numeric consistency audit (proof-complete revision of "
       ...
       "counts_ledger": [{"value": v, "meaning": m, "source": s}
                         for v, m, s in ledger],
       "checks": checks}
with open(os.path.join(DB, "v8_number_audit.json"), "w") as f:
...
with open(os.path.join(DB, "v8_number_audit.md"), "w") as f:
    f.write("\n".join(md))
