#!/bin/bash
# Build self-contained, compile-ready submission ZIPs for the v11 round:
# main = journal_manuscript_v8.tex (BMB), companion = companion_categorical_v7.tex (TAC).
# Changes vs build_submission_zips_v10.sh (v10 round):
#   - New manuscript versions (versioned files, prior versions untouched):
#     journal_manuscript_v8.tex (from v7) and companion_categorical_v7.tex
#     (from v6).
#   - Merged-synthesis round (targeted accessibility grafts from the
#     evaluated external humanization attempts, plan at
#     download/Humanized_Versions_Evaluation_and_Synthesis_Plan.md):
#     main -- abstract robustness clauses compressed to one sentence with
#     the numbers kept in the body and the n=424 parenthetical
#     disambiguated (238 words); value-function gloss ("maximum
#     achievable growth rate"); zero-cost-substitution gloss inside the
#     coupling theorem; Discussion layer-separation sentence; the
#     98->301 check-count reconciliation; two process-flavored remark
#     titles recast ("Status of the definition" -> "Definition, ...";
#     "What this closes" -> "The exact value--flux relation").
#     Companion -- the boundary-rule three-fold-failure breakdown; the
#     thermostat/cell autopoiesis contrast; the Zeno physical reading;
#     proof-provenance recasts ("self-contained" -> "given in full";
#     "definitional rhetoric" removed).
...
STAGE=scripts/overleaf_stage_v11
rm -rf "$STAGE"
mkdir -p "$STAGE/main" "$STAGE/companion"

# ---------- MAIN (Bulletin of Mathematical Biology) ----------
M=$STAGE/main
cp scripts/journal_manuscript_v8.tex          "$M/"
cp scripts/journal_manuscript_v8_bmb_refs.tex "$M/"
cp scripts/journal_manuscript_v8_refs.bib     "$M/"
...
- Overleaf: upload this ZIP via New Project -> Upload Project, set the main
  document to journal_manuscript_v8.tex, recompile. (Run twice so natbib
  citations resolve; Overleaf does this automatically.)
- Command line: pdflatex journal_manuscript_v8.tex  (x2)

## Contents
- journal_manuscript_v8.tex        main source (main document)
- journal_manuscript_v8_bmb_refs.tex  reference list (input by the main file)
- journal_manuscript_v8_refs.bib   underlying BibTeX database (not required
...
- Abstract: 238 words (within the 150-250 guideline).
...
rm -rf /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v11 && mkdir -p /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v11/main /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v11/companion
unzip -q download/submission_main_bmb.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v11/main
unzip -q download/submission_companion_tac.zip -d /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v11/companion
echo "=== fresh-dir compile: main v8 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v11/main && tectonic journal_manuscript_v8.tex 2>&1 | tail -2 && \
 pdfinfo journal_manuscript_v8.pdf | grep Pages)
echo "=== fresh-dir compile: companion v7 ==="
(cd /home/z/my-project/metabolic-curvature-measure/scripts/zipcheck_v11/companion && tectonic companion_categorical_v7.tex 2>&1 | tail -2 && \
