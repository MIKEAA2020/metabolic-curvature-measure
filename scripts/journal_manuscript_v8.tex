...
%  standalone theory companion (scripts/companion_categorical_v7.tex),
...
tie-breaking convention and to random-panel sampling. The
association does not propagate to the
protein layer: cells transcribe rerouting potential while buffering
its translation. Double-knockout epistasis aligns with active-set
...
When the parameter path crosses a switching boundary, the optimal
flux vector changes slope abruptly. The following definition collects
...
measured metric invariance of \S\ref{sec:v5}. The empirical
...
The study is correlational by design, and its scope is a single
organism. The active sets used throughout are operational (obtained
from solved LPs) rather than combinatorial. The lexicographic
tie-breaking is an engine convention: the value-function carrier is
immune to it, the flux layer declares it (Remark~\ref{rem:lock}), and
the association is stable across all five selection rules tested
(declared plus four variants, \S\ref{sec:v8}). The one-chamber
structure of the reference path is condition-specific
(\S\ref{sec:v7}), while the layer decision itself is path-robust.
The cross-platform carbon-switch dissociation (PRECISE arm,
\S\ref{sec:v5}) bounds the generality of the association. The
active-reaction census of the reference physiology ($438/2{,}583$
...
stable (Appendix~\ref{sec:counts}). The sweep arm of the
stabilization experiment is panel-limited (thirteen sweep families,
two of them affine --- the finite-population correction is part of
the reported result), and the two-dimensional arm is floor-limited at
large panels by the transport-LP atom cap.
...
single workstation. An automated numeric verification of $301$ checks
re-derives every
manuscript number from the deposited artifacts and is
...
\input{journal_manuscript_v8_bmb_refs}
