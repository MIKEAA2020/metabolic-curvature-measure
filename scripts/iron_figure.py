#!/usr/bin/env python3
"""Publication figure for the iron-axis probe: 4 panels, matching
the nitrogen/phosphate-axis figure layout.

(a) label flips vs glucose-only baseline per level (all zero on this
    axis -- the trace-metal supply closure)
(b) wild-type growth along the axis (37% -> 84% reduction; identical
    linear dose response on both reconstructions)
...
download/keio_iron_limited_response.png.
"""
import os, json
import numpy as np
import matplotlib
...
ctrl = json.load(open(os.path.join(DL, "keio_iron_pfba_control.json")))
...
IJO_KEYS = ["fe_0.01", "fe_0.005", "fe_0.0025"]
IML_KEYS = ["fe_0.005", "fe_0.0025"]
x_labels = ["baseline\n(Fe unlim.)", "Fe $-0.01$", "Fe $-0.005$",
            "Fe $-0.0025$"]
...
axA.set_title("(a) Label re-stratification\n(zero at every level: the trace-metal side)",
              fontsize=10)
...
# the identical iML dose response (shared quota; b = fe2/0.0161)
iml_wts = [iml_res["levels"][k]["wild_type_biomass"] for k in IML_KEYS]
axB.plot(xs[2:], iml_wts, '*', ms=11, color='purple', alpha=0.6,
         label='iML1515 (identical to 6 decimals)')
...
axB.set_title("(b) Growth along the iron axis\n(84% reduction; shared quota, linear map)",
              fontsize=10)
...
fig.suptitle("Fifth perturbation axis: iron limitation "
             "(single ferrous channel; iJO1366 gradient, iML1515 cross-rebuild)",
             fontsize=12, fontweight="bold")
plt.savefig(os.path.join(DL, "keio_iron_limited_response.png"), dpi=120)
plt.close()
print("iron figure written.")
